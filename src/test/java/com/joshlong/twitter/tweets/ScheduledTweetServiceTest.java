...
class ScheduledTweetServiceTest {
	...
	/*
	 * @Test void unscheduled() { var json = """ { "text" : "Hi, Spring fans!" } """; var
	 * tomorrow = Date.from(Instant.now().minus(10, ChronoUnit.MINUTES)); var scheduled =
	 * this.scs.schedule("@TEST_TWITTER_USERNAME", json, tomorrow, "test_client",
	 * this.password, null); var unscheduled = this.scs.due(); var match =
	 * unscheduled.filter(st -> st.scheduled().equals(tomorrow) &&
	 * st.clientId().equals("test_client") && st.clientSecret().equals(this.password) &&
	 * st.username().equals("test_twitter_username") &&
	 * st.jsonRequest().equals(json.trim()));
	 * StepVerifier.create(scheduled.thenMany(match))// .expectNextCount(1)//
	 * .verifyComplete(); var unscheduledAfterSending = this.scs.schedule(new
	 * ScheduledTweet("@TEST_TWITTER_USernAME", json, tomorrow, "test_client",
	 * this.password, null, "242232"), new Date()).thenMany(this.scs.due());
